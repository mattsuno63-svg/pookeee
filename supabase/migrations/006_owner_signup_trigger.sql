-- Gestore: crea owner e store nel trigger handle_new_user (funziona anche con conferma email)
...
BEGIN
  ...
  IF v_role = 'owner' THEN
    ...
    END IF;
    -- Unicità: se esiste già, aggiungi suffisso
    ...
    INSERT INTO public.owners (
      id, business_name, vat_number, business_email, business_phone,
      trial_ends_at, application_status
    ) VALUES (
      NEW.id,
      v_business_name,
      NULLIF(trim(v_meta->>'vat_number'), ''),
      NULLIF(trim(v_meta->>'business_email'), ''),
      NULLIF(trim(v_meta->>'business_phone'), ''),
      v_trial_ends,
      'pending'
    );

    INSERT INTO public.stores (
      owner_id, name, slug, description, address, city, postal_code,
      phone, email, social_links
    ) VALUES (
      NEW.id,
      v_business_name,
      v_slug,
      NULLIF(trim(v_meta->>'description'), ''),
      NULLIF(trim(v_meta->>'address'), ''),
      NULLIF(trim(v_meta->>'city'), ''),
      NULLIF(trim(v_meta->>'postal_code'), ''),
      COALESCE(NULLIF(trim(v_meta->>'phone'), ''), NULLIF(trim(v_meta->>'business_phone'), '')),
