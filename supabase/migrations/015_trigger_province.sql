-- Aggiungi province al trigger handle_new_user
...
BEGIN
  ...
  v_province := NULLIF(TRIM(v_meta->>'province'), '');
  v_lat := (v_meta->>'latitude')::DECIMAL;
  v_lon := (v_meta->>'longitude')::DECIMAL;

  INSERT INTO public.profiles (id, role, nickname, city, province, latitude, longitude)
  VALUES (
    NEW.id,
    v_role,
  ...
  IF v_role = 'owner' THEN
    ...
    INSERT INTO public.stores (owner_id, name, slug, description, address, city, province, postal_code, phone, email, social_links)
    VALUES (NEW.id, v_business_name, v_slug, NULLIF(trim(v_meta->>'description'), ''), NULLIF(trim(v_meta->>'address'), ''), NULLIF(trim(v_meta->>'city'), ''), NULLIF(trim(v_meta->>'province'), ''), NULLIF(trim(v_meta->>'postal_code'), ''), COALESCE(NULLIF(trim(v_meta->>'phone'), ''), NULLIF(trim(v_meta->>'business_phone'), '')), COALESCE(NULLIF(trim(v_meta->>'emailStore'), ''), NULLIF(trim(v_meta->>'business_email'), '')), v_social);
  END IF;
